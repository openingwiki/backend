...
async def register(
    wiki_page_data: Annotated[schemas.WikiPageAdd, Body()],
    db: Session = Depends(dependencies.get_db),
    user: models.User = Depends(dependencies.get_current_user),
):
    """
    Adding wiki pages. Geting user token from cookie.
    If user is moderator, then adding straightaway
    Else request will be sent to moderation.

    Parameters:
        wiki_page_data: Annotated[schemas.WikiPageAdd, Body()] - wiki_page data in body with json
    ...
    if user.is_moderator:
        ...
        crud_wiki_page.add_wiki_page(db, wiki_page_data, needs_moderation=False)
        return {"added": True}

    else:
        wiki_page_data.added_by_user = user.id
        crud_wiki_page.add_wiki_page(db, wiki_page_data, needs_moderation=True)
